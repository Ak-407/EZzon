...
app.get("/", function(req, res) {
  file.find({}, function(err, foundItems) {
    kitchen.find({}, function(err, foundItemsk) {
    phone.find({}, function(err, foundItemsp) {
    skill.find({}, function(err, skillItems) {
      res.render("header", { recordss: foundItems, skillName: skillItems, kitchen:foundItemsk, phone:foundItemsp });
    });
  })})});
});
...
app.post("/kitchen", upload, function(req, res) {
  ...
  kit.save(function(err, foundItems) {
    if (!err) {
      res.redirect("/"); // Redirect to the home page
    } else {
      res.send(err);
...
app.post("/phone", upload, function(req, res) {
  ...
  pho.save(function(err, foundItems) {
    if (!err) {
      res.redirect("/"); // Redirect to the home page
    } else {
      res.send(err);
